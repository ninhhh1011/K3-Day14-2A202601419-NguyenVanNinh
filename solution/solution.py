...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            rel = (overlap / len(expected_tokens)) >= relevance_threshold
...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        if len(scores_batch) > 1:
            ...
            if first_item_scores and rest_scores:
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list[EvalResult], baseline_results: list[EvalResult]) -> dict[str, Any]:
        ...
        """
        def _get_avg(results: list[EvalResult], attr: str) -> float:
            if not results:
                return 0.0
            return sum(getattr(r, attr) for r in results) / len(results)

        new_f = _get_avg(new_results, "faithfulness")
        new_r = _get_avg(new_results, "relevance")
        new_c = _get_avg(new_results, "completeness")

        base_f = _get_avg(baseline_results, "faithfulness")
        base_r = _get_avg(baseline_results, "relevance")
        base_c = _get_avg(baseline_results, "completeness")
    ...
    def identify_failures(
        ...
        for r in results:
            if not r.passed or r.faithfulness < threshold or r.relevance < threshold or r.completeness < threshold:
                failures.append(r)
        return failures
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        min_val = min(f, r, c)
        if f == min_val and f < 0.5:
            return "Context is missing or irrelevant — improve retrieval"
        elif r == min_val and r < 0.5:
            return "Answer does not address the question — improve prompt clarity"
        elif c == min_val and c < 0.5:
    ...
    def generate_improvement_suggestions(
        ...
        if not failures:
            return [
                "Pipeline is performing well — maintain current guardrails and monitoring",
                "Expand golden dataset with more hard and adversarial test cases",
                "Benchmark against next-generation model versions"
            ]
        ...
        if counts.get("hallucination", 0) > 0:
            ...
            suggestions.append("Refine system prompt to strictly prohibit using external knowledge outside retrieved context")
        if counts.get("irrelevant", 0) > 0 or counts.get("off_topic", 0) > 0:
            suggestions.append("Improve prompt clarity and intent classification to handle user query nuances")
        ...
        default_pool = [
            ...
            "Add few-shot examples showing complete answers to improve completeness",
            "Implement hallucination checker to filter unsupported claims",
            "Refine BM25 retriever top-k parameter to improve evidence retrieval"
        ]
    ...
    def generate_improvement_log(self, failures: list[EvalResult], suggestions: list[str]) -> str:
        ...
        for index, f in enumerate(failures, start=1):
            fid = f"F{index:03d}"
            ftype = f.failure_type or "Unknown"
            rcause = self.find_root_cause(f)
            sfix = suggestions[index - 1] if index - 1 < len(suggestions) else (suggestions[0] if suggestions else "Investigate pipeline")
